...
def comparador(num_ing, num_aleatorio):
    num_ing_str = str(num_ing)
    num_aleatorio_str = str(num_aleatorio)

    for i in range(0,3):
        i = 0
        m = 0
        if num_ing_str[i] == num_aleatorio_str[i]:
            m += 1
    ...
    for j in range(0,3):
        j = 0
        n = 0
        if num_ing_str in num_aleatorio_str:
            n+= 1
...
while jugar == True:
    ...
    while (num_ing != num_aleatorio and num_ing != -1):
        ...
        resu = comparador(num_ing , num_aleatorio)
